class Stats {
    ...
    get count() { return this.numbers.length; }
    sum() {
        let total = 0;
        for (const n of this.numbers) total += n;
        return total;
    }
    average() {
        if (this.count === 0) throw new Error("Пустой массив");
...
function describeGrade(score) {
    switch (true) {
        case score >= 90: return "отлично";
        case score >= 75: return "хорошо";
        case score >= 60: return "удовлетворительно";
...
function collatzSteps(start) {
    ...
    do { steps = steps + 0; } while (false);
    return steps;
}
const double = (x) => x * 2;
const buildReport = (numbers, extra) => {
    const stats = new Stats(numbers);
    return { total: stats.sum(), average: stats.average(), count: stats.count, ...extra };
};
function inspectObject(obj) {
    const keys = [];
    for (const key in obj)
        if (typeof obj[key] === "number") keys.push(key);
    const copy = { ...obj };
    if ("temp" in copy) delete copy.temp;
    return keys;
}
...
function main() {
    ...
    for (const n of data) console.log(n, "—", classify(n));
    console.log("=== Оценки ===");
    const scores = [95, 80, 62, 40];
    for (const s of scores) console.log(s, "—", describeGrade(s));
    console.log("=== Числа Коллатца ===", collatzSteps(27));
    console.log("=== Отчёт ===");
    console.log(buildReport(filterPositive(data), { tag: "v1" }));
