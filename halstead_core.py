...
# Односимвольные операторы. '.' ';' и ',' ВКЛЮЧЕНЫ.
# Запятая считается ВСЕГДА, без исключений:
#   foo(a, b)            аргументы вызова
#   [1, 2, 3]            литерал массива
#   {a: 1, b: 2}         литерал объекта
#   x = (f(), g())       оператор «запятая»
#   for (i=0, j=1; …)    секции заголовка for
#   let a = 1, b = 2;    список объявляемых переменных
#   function f(a, b)     формальные параметры
#   (a, b) => a + b      параметры стрелочной функции
#   const {a, b} = o;    деструктуризация
#   import {a, b} from … список импорта
...
def analyze(code: str) -> HalsteadResult:
    ...
    for idx, tok in enumerate(tokens):
        ...
        if kind == "PUNCT":
            ...
            # запятая — оператор во всех контекстах без исключений
            if text == COMMA:
                result.add_operator(COMMA)
                continue
